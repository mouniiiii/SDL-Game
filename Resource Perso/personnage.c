...
{ 
  ...
  if(p->deplacement==1 && p->position_perso.x >500 && p->position_perso.x <510)//SI LE HERO DEPASSE LE MILIEU DU SCREEN ON LUI RENVOIE
         {								      // A SA POSITION INITIALE
           p->x =60;
 	   }
        if(p->position_perso.x>300)//position de l'ennemi 
          {
